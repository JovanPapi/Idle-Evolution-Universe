...
class UpgradesFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val upgrade_section_adapter = UpgradeElementHomeAdapter(result_list_section, object : OpenUpgradeSectionListener{
            override fun openUpgradeSection(curr_section: Section) {
                val bundle = Bundle()
                bundle.putString("section_name", curr_section.name)
                findNavController().navigate(R.id.action_navigation_upgrades_to_showUpgradeSectionElementsFragment, bundle)
            }
        })
